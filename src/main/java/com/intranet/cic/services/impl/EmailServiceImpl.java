...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendNewTicketNotification(String ticketNumber, String title,
        ...
        try {
            ...
            // ✅ Attach local files directly — no temp files needed
            if (attachmentUrls != null) {
                for (String url : attachmentUrls) {
                    try {
                        Path filePath = fileStorageService.resolveFilePath(url);
                        String filename = url.substring(url.lastIndexOf('/') + 1);
                        helper.addAttachment(filename, filePath.toFile());
                        log.info("Attached local file: {}", filename);
                    } catch (Exception e) {
                        log.warn("Could not attach file {}: {}", url, e.getMessage());
                    }
                }
            }
